...
export class WebsitesService {
  ...
  static async getAllWebsites(filters = {}) {
    ...
    if (search) {
      sql += ` AND (w.website_name ILIKE $${paramIndex} OR w.website_url ILIKE $${paramIndex} OR c.name ILIKE $${paramIndex} OR c.company ILIKE $${paramIndex})`;
      params.push(`%${search}%`);
      paramIndex++;
    }

    if (client_id) {
      sql += ` AND w.client_id = $${paramIndex}`;
      params.push(client_id);
      paramIndex++;
    }

    if (technology) {
      sql += ` AND w.technology = $${paramIndex}`;
      params.push(technology);
      paramIndex++;
    }

    if (priority) {
      sql += ` AND w.priority = $${paramIndex}`;
  ...
  static async getWebsiteById(id) {
    ...
    const logsResult = await query(
      `SELECT * FROM maintenance_logs WHERE website_id = $1 ORDER BY performed_at DESC LIMIT 10`,
      [id]
    );
  ...
  static async updateWebsite(id, payload) {
    return transaction(async (client) => {
      const existing = await client.query(`SELECT id FROM websites WHERE id = $1`, [id]);
      ...
      await client.query(
        `UPDATE websites
         SET 
           ...
           website_name = COALESCE($2, website_name),
           website_url = COALESCE($3, website_url),
           technology = COALESCE($4, technology),
           hosting_provider = COALESCE($5, hosting_provider),
           domain_provider = COALESCE($6, domain_provider),
           date_built = COALESCE($7, date_built),
           support_plan = COALESCE($8, support_plan),
      ...
      if (hosting) {
        await client.query(
          `INSERT INTO hosting_details (website_id, provider, plan_name, renewal_date, cost, auto_renew, notes)
           ...
           SET 
             provider = COALESCE(EXCLUDED.provider, hosting_details.provider),
             plan_name = COALESCE(EXCLUDED.plan_name, hosting_details.plan_name),
             renewal_date = COALESCE(EXCLUDED.renewal_date, hosting_details.renewal_date),
             cost = COALESCE(EXCLUDED.cost, hosting_details.cost),
             auto_renew = COALESCE(EXCLUDED.auto_renew, hosting_details.auto_renew),
             notes = COALESCE(EXCLUDED.notes, hosting_details.notes),
             updated_at = CURRENT_TIMESTAMP`,
          [
            id,
            hosting.provider,
      ...
      if (domain) {
        await client.query(
          `INSERT INTO domain_details (website_id, registrar, expiry_date, cost, auto_renew, notes)
           ...
           ON CONFLICT (website_id) DO UPDATE
           SET 
             registrar = COALESCE(EXCLUDED.registrar, domain_details.registrar),
             expiry_date = COALESCE(EXCLUDED.expiry_date, domain_details.expiry_date),
             cost = COALESCE(EXCLUDED.cost, domain_details.cost),
             auto_renew = COALESCE(EXCLUDED.auto_renew, domain_details.auto_renew),
             notes = COALESCE(EXCLUDED.notes, domain_details.notes),
             updated_at = CURRENT_TIMESTAMP`,
          [
            id,
            domain.registrar,
      ...
      if (ssl) {
        await client.query(
          `INSERT INTO ssl_details (website_id, issuer, expiry_date, ssl_type, auto_renew, notes)
           ...
           ON CONFLICT (website_id) DO UPDATE
           SET 
             issuer = COALESCE(EXCLUDED.issuer, ssl_details.issuer),
             expiry_date = COALESCE(EXCLUDED.expiry_date, ssl_details.expiry_date),
             ssl_type = COALESCE(EXCLUDED.ssl_type, ssl_details.ssl_type),
             auto_renew = COALESCE(EXCLUDED.auto_renew, ssl_details.auto_renew),
             notes = COALESCE(EXCLUDED.notes, ssl_details.notes),
             updated_at = CURRENT_TIMESTAMP`,
          [
            id,
            ssl.issuer,
      ...
      if (email_cdn) {
        await client.query(
          `INSERT INTO email_cdn_details (website_id, email_provider, email_expiry_date, cdn_provider, cdn_expiry_date, notes)
           ...
           ON CONFLICT (website_id) DO UPDATE
           SET 
             email_provider = COALESCE(EXCLUDED.email_provider, email_cdn_details.email_provider),
             email_expiry_date = COALESCE(EXCLUDED.email_expiry_date, email_cdn_details.email_expiry_date),
             cdn_provider = COALESCE(EXCLUDED.cdn_provider, email_cdn_details.cdn_provider),
             cdn_expiry_date = COALESCE(EXCLUDED.cdn_expiry_date, email_cdn_details.cdn_expiry_date),
             notes = COALESCE(EXCLUDED.notes, email_cdn_details.notes),
             updated_at = CURRENT_TIMESTAMP`,
          [
            id,
  ...
  static async archiveWebsite(id) {
    const result = await query(
      `UPDATE websites SET is_archived = TRUE, archived_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING id, website_name, is_archived`,
      [id]
    );
    if (result.rowCount === 0) {
      throw new Error('Website not found.');
  ...
  static async restoreWebsite(id) {
    const result = await query(
      `UPDATE websites SET is_archived = FALSE, archived_at = NULL WHERE id = $1 RETURNING id, website_name, is_archived`,
      [id]
    );
    if (result.rowCount === 0) {
      throw new Error('Website not found.');
